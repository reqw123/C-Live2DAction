...
- **元培 boss 魂刃劍氣 SoulBladeQi（2026-09-06 續190 → 續191 重做，見 `Docs/YUANPEI_LOGO_SKY_BOSS.md`）**：`YuanpeiBoss.attackPool` 第 10 招。續190 是 `YuanpeiProjectile` 飛劍；**續191 使用者改成「站樁伸縮長矛」**：`YuanpeiSoulBladeLance.cs`（新）—— 細亮光桿（Shaft）+ 固定長度 flipbook 刀頭（Head，`SoulBladeQi_Atlas.png` 12×6/72、繞軸 billboard、`_MainTex` Tiling/Offset 逐格）整體沿鎖定方向 0→reach 延伸，尖端到位爆散；命中 = `YuanpeiAttacks.SoulBladeQi` 每幀 `RayHitsPlayer` 線段判定（側走可閃）。one-shot（count 1）。數值全在 `YuanpeiAttack_SoulBladeQi.asset`：`number1`=延伸時長 0.5s、`number2`=刀身寬 2.6m、`number3`=爆散時長 0.28s、`number5`=命中半徑 1.0m；phase 2+、5–28m、cd 8s、46 傷。選單 `Setup Yuanpei Soul Blade Qi (魂刃劍氣)`。續191b：`requiredPhase` 2→**1**（正式戰抽得到）。續191c：前搖改三拍 `SoulBladeArmAndSnap` —— A 圓盤本體 additive halo 充能（「身體上的準備發射預警」）+ 9 mote 螺旋收束、B **隨機待命 1~3s**（`number4`=最小秒數）圓盤鬆散朝玩家轉但不鎖定、C snap 白閃 → **snap 結束才鎖定+發射**。`telegraphSeconds` 1.0→0.4。編譯無錯、選單跑成功、Play（凍幀 Step）截圖驗延伸/billboard/逐格/halo/mote 正確。**待對焦 Play**（F8 數字鍵 0）：延伸速度 / 刀身寬 / **shaft + halo 亮度顏色（偏亮偏白）** / 1~3s 待命緊張感 / 發射瞬間明確度 / 爆散讀感 / 命中線寬。
- **ESC 跳過 boss 開場動畫（2026-09-07 續191b）**：`YuanpeiIntroCinematic`（元培，`SkipToFightStart`）與 `BossIntroManager`（武士 Timeline，`SkipIntro`）都在 update 迴圈查 `Keyboard.current.escapeKey`，跳到戰鬥開始狀態後照常交接。開關 `allowEscSkip`（預設 on）。編譯無錯。**待對焦 Play**：兩隻 boss 按 ESC 後 boss 姿勢/相機/玩家動畫的交接乾淨度（凍幀驗不出動畫）。
- **下馬威前搖不再震動（2026-09-07 續191b）**：`RunOpeningBarrage` telegraph 迴圈每幀的 `shake?.Shake` 已移除（使用者：技能還沒射出時不要震動）；震動只剩發射瞬間 + 打完地面環標點。
...
6b. **`Map_Camp`（露營區，2026-09-11 新增）**：第一個不從 `BoundaryWall_*` 缺口接出的區域場景——本地四面牆已各自對應學校/二次元/現世，北牆貼武士 boss 戰鬥區沒空間，改成從既有 `VehicleRoad_West`（x −15~−85）中段 x≈−50 分岔一條 T 字支線 `VehicleRoad_Camp`（z 0~−70），`CampGate_Enter`(−50,0,−67)/`CampGate_Exit`(−50,0,−71,於 `Map_Camp` 內) 完全比照 `SchoolGate_Enter`/`Exit` 的元件結構（`PortalSurface` 漩渦影片 quad + `Blocker` + F 互動提示），只是各自一組獨立的 `RT_CampGate_*.renderTexture`/`Mat_CampGate_*.mat`。`露營區` ground 60×60 + `CampWall_*` ×5（南/東/西封閉、北牆 Left/Right 留中央缺口給支線），座標是把 `Map_School` 的 學校/`SchoolWall_*` 整組平移 (dx=−50, dz=+15) 得到，數值上跟學校場地在世界座標裡有小範圍重疊（不同場景，正常遊玩不會同時載入，純數字巧合，非 bug）。**這次只做場地**：沒有任何角色/建築/互動物件，新角色「猜猜看」還沒有素材，先不建立位；有素材時依 `Docs/ASSET_LICENSES.md` 追蹤授權。**待使用者對焦 Editor Play 驗證**（本次用 Unity MCP 建物件+存檔+座標核對+Scene View screenshot 驗證幾何，`execute_code` 進 Play Mode 時編輯器缺 OS focus 導致 `Time.frameCount` 卡住、協程不跑，沒能跑完整個 `SceneTransitionRunner` 流程）：沿 `VehicleRoad_West` 走到 x≈−50 左轉南下 → `CampGate_Enter` 前出現「按下 F 進入露營區」→ 按 F → 淡黑「載入中…」→ 站在 `露營區` 內 (−50,1.1,−77) 面向南 → 走到 `CampGate_Exit` → 按 F → 淡黑 → 回到支線路上 (−50,1.1,−63) 面向北、`Map_Camp` 卸載。

6c. **「猜猜看」升級成第三個可附身角色（2026-09-11 續，G 鍵）**：`GuessWhoPlayerParitySetup.cs` 整包複製 Player 再換皮（Humanoid 骨架自動對應成功，共用 `NewAnimator_PlayerImmersiveWalk`），移動/攻擊連段/F 防禦架勢/R 必殺技/處決元件全部到位，`CameraPossessionSwitcher` 擴充成三方（G=猜猜看，C 維持 Player↔Cat 嚴格語意，在猜猜看身上按 C 直接回 Player）。
   - **2026-09-11 續 2（同日追加）**：拿掉背後純裝飾的 `BackGreatswordDecor`（狼大劍複製品，留著右手武器 `WolfsGravestone`/血刀不動——`UltimateAbility.FindWeapon()` 靠這個名字找武器，拿掉 R 必殺技會安靜地整個失效不報錯）；新增 `HumanoidWalkArmsDown.cs`（alt 慢走時用 Humanoid 肌肉空間覆寫雙臂成下垂打直，蓋掉共用走路動畫在他身上顯得奇怪的擺臂，只在 `IsWalking` 時生效，Player 不受影響）；用 `GuessWhoCampRelocateSetup.cs` 把他＋專屬攝影機從常駐 `GreyboxTest` 搬進 `Map_Camp.unity`，站在「露營區」地板上，同時砍掉先前純視覺佔位那份重複的「猜猜看」。
   - **搬去 `Map_Camp` 的技術後果**：他現在只存在於按需載入的區域場景，`CameraPossessionSwitcher`（在常駐場景）的 `guessWhoCamera`/`guessWhoControl`/`guessWhoHealth` 已清空，改成執行期用名字重新找（`TryRelinkGuessWho()`，每秒節流掃描）。**結果：G 鍵要等玩家實際走去過露營區一次（`Map_Camp` 真的載入）才會生效**，在那之前按 G 沒反應（不報錯，設計上合理，不是 bug）。
   - **撤回先前的誤判**：原本以為下半身是 Meshy 生成的大圓裙造型、擔心格擋/踢腿穿模——那其實是還沒清掉的 `Icosphere` 邊界球造成的視覺錯覺，拿掉後其實是正常比例人形（灰連帽衣/黑褲/黃靴）。
   - **已知待調**：武器握把貼合度（`RightHand` 骨頭 `lossyScale=100`，跟 memory 記錄的「80x bone scale」同類陷阱）未精修；攝影機沿用 Player 的調校值當起點，比例是否適合他還沒對焦 Play 確認；`HumanoidWalkArmsDown` 的肌肉數值（`armDownUp=-0.9` 等）是估的起始值。**踩到的坑**：`PrefabUtility.InstantiatePrefab` 生出的實例階層鎖死不給重排子物件（要 `UnpackPrefabInstance` 解鎖）。
   - **待使用者對焦 Editor Play 驗證**：G 鍵在露營區內外的實際行為、移動/攻擊/F/R 是否正常運作、alt 慢走手臂覆寫效果（本次因編輯器缺 OS focus 沒能實測，只核對過所有欄位/元件的靜態接線）。
