...
// Enemy always walked all the way to near-melee distance before ever attacking, regardless
...
// cut down to 1.5, so Enemy declared Attacking from a distance its actual hit capsule
...
// 0.5/0.5 for a melee-feeling Enemy, 2026-08-16 user decision) - it reads whatever
// Range/Radius are currently configured and asserts Enemy only ever lands a hit once within
...
public class EnemyAttackRangeSceneTests
{
    ...
    [UnityTest]
    public IEnumerator Enemy_AttacksPlayerOnlyOnceWithinItsOwnAttackCapsuleReach_InRealScene()
    {
        ...
        Assert.IsNotNull(ai, "Enemy has no EnemyAI");

        PlayerCombat combat = enemy.GetComponent<PlayerCombat>();
        Assert.IsNotNull(combat, "Enemy has no PlayerCombat");
        AttackData attack = combat.PrimaryAttack;
        Assert.IsNotNull(attack, "Enemy's PlayerCombat has no PrimaryAttack configured");
        float effectiveRange = attack.Range + attack.Radius;

        // Comfortably beyond the current effective range so Enemy actually has to chase in,
        ...
        player.transform.position = enemy.transform.position + direction * testDistance;
        ...
        {
            float currentDistance = Vector3.Distance(player.transform.position, enemy.transform.position);
        ...
        Assert.Less(playerHealth.CurrentHealth, startingHealth, "Enemy should have landed a hit within 3s");
        Assert.GreaterOrEqual(minDistanceObserved, effectiveRange - CapsuleReachTolerance,
            $"Enemy's attack-commit distance looks desynced from its actual hit capsule reach ({effectiveRange:F2}) - closest observed: {minDistanceObserved:F2}");
    }
}
