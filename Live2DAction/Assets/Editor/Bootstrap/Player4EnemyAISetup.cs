...
{
    // Converts Player4 (the static "Arisa" standee added by Player4AnimeVisualSetup.cs) into
    // an AI-driven enemy, reusing the exact same EnemyAI/Health/PlayerCombat wiring
    // GreyboxSceneBuilder.CreateEnemy used for the now-deleted TrainingDummy (2026-08-12,
    // explicit user request: "把 Player4 當作敵人開始製作 AI 自主攻擊模式").
    //
    // Unlike TrainingDummy (whose detectionRange was deliberately zeroed to make it a
    // stationary target), this leaves EnemyAI's detectionRange/attackRange/moveSpeed at the
    // class's own defaults (8/2/2) - the point this time is a real autonomous chase-and-attack
    // enemy, not a punching bag.
    //
    // Standee -> CharacterController swap changes the coordinate frame Player4's root sits
    ...
    internal static class Player4EnemyAISetup
    {
        ...
        public static void Apply()
        {
            ...
            {
                Debug.LogError("Player4 or Player GameObject not found in " + ScenePath);
                return;
            }
            ...
            CapsuleCollider existingCollider = player4.GetComponent<CapsuleCollider>();
            if (existingCollider != null)
            {
                Object.DestroyImmediate(existingCollider);
            }

            CharacterController controller = player4.AddComponent<CharacterController>();
            ...
            CapsuleCollider capsuleReference = player4.AddComponent<CapsuleCollider>();
            ...
            player4.transform.position = new Vector3(
                player4.transform.position.x,
                groundTopY + controller.center.y + controller.height / 2f,
                player4.transform.position.z);

            Transform visual = player4.transform.Find("Visual");
            if (visual != null)
            {
                visual.localPosition = new Vector3(0f, controller.center.y - controller.height / 2f, 0f);
            }

            if (player4.GetComponent<Health>() == null)
            {
                player4.AddComponent<Health>();
            }

            EnemyAI ai = player4.GetComponent<EnemyAI>();
            if (ai == null)
            {
                ai = player4.AddComponent<EnemyAI>();
            }
            ...
            if (combat == null)
            {
                combat = player4.AddComponent<PlayerCombat>();
            }
            ...
            // LockOnTarget was already added by Player4AnimeVisualSetup.cs - left as-is.

            EditorSceneManager.SaveScene(scene);
            AssetDatabase.SaveAssets();
            Debug.Log("Converted Player4 into an AI-driven enemy (EnemyAI + Health + PlayerCombat, reusing EnemyAttack.asset).");
        }
    }
}
