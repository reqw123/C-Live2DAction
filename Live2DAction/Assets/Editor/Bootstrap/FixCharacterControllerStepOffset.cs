...
namespace Live2DAction.EditorTools
{
    ...
    // stepOffset (0.3), walking Player straight into Enemy (also a CharacterController) let
    ...
    // avoidance - see KNOWN_ISSUES.md - so it likely ended up clipped into Enemy's head
    // geometry from up there). stepOffset=0 stops the climb entirely (confirmed: Y stayed
    // exactly flat across the same test). GreyboxSceneBuilder.cs/EnemyAISetup.cs's
    ...
    internal static class FixCharacterControllerStepOffset
    {
        ...
        public static void Apply()
        {
            ...
            int fixedCount = 0;
            foreach (string name in new[] { "Player", "Enemy", "TrainingDummy" })
            {
