...
namespace Live2DAction.EditorTools
{
    ...
    // can be hit, same behavior class as Mecha (explicit user confirmation: damageable like
    // Mecha, not an untouchable prop).
    //
    ...
    // choice Mecha already made for the same "purely passive" reason.
    internal static class TrainingDummySetup
    {
        ...
        [MenuItem("Tools/Live2DAction/Add TrainingDummy (Stationary Damageable Dummy, Maya Visual)")]
        public static void Apply()
        {
            ...
            {
                Debug.LogError("TrainingDummy already exists in " + ScenePath);
                return;
            }
            ...
            var trainingDummy = new GameObject("TrainingDummy");
            ...
            // to drive - matches Mecha's own reasoning for the same choice.
            CapsuleCollider collider = trainingDummy.AddComponent<CapsuleCollider>();
            collider.center = Vector3.zero;
            collider.radius = 0.5f;
            collider.height = 1f;

            trainingDummy.transform.position = new Vector3(5f, groundTopY + collider.center.y + collider.height / 2f, 0f);

            trainingDummy.AddComponent<Health>();

            PlayerCombat combat = trainingDummy.AddComponent<PlayerCombat>();
            ...
            GameObject visual = (GameObject)PrefabUtility.InstantiatePrefab(prefabAsset, trainingDummy.transform);
            visual.name = "Visual";
            ...
            CharacterAttackAnimationLink link = trainingDummy.AddComponent<CharacterAttackAnimationLink>();
            ...
            // Mecha parity (explicit user confirmation): lockable and shows a health bar,
            // same as any other damageable-but-passive character.
            trainingDummy.AddComponent<LockOnTarget>();
            HealthBarSetup.AddHealthBar(trainingDummy);

            EditorSceneManager.SaveScene(scene);
            AssetDatabase.SaveAssets();
            Debug.Log("Added TrainingDummy - stationary, damageable Maya-visual dummy sharing Player's exact LightAttack1/2/3 combat setup; never moves or attacks (no input source, no AI).");
        }
    }
}
